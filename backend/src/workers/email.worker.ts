...
const worker = new Worker<EmailJobData>(
  ...
  async (job: Job<EmailJobData>) => {
    ...
    if (email.status === "PROCESSING") {
      console.log(
        `Email ${emailId} is already processing. Skipping.`
      );
    ...
    if (
      ...
      await sendSlackRateLimitNotification(
        email.userId,
        `⚠️ ReachInbox rate limit reached: sender ${email.sender.email} has reached ${hourlyLimit} emails for the current hour. Remaining scheduled emails will be delayed until the next hour.`
      );
    }
    ...
    if (!rate.allowed) {
      ...
      const replacementJobId =
        `email-${emailId}-window-${nextHour}`;

      console.log(
        `Hourly limit reached for ${email.sender.email}. Rescheduling ${emailId}.`
      );
    ...
    try {
      console.log(
        `Sending ${email.recipient} from ${email.sender.email}`
      );
...
worker.on("ready", () => {
  ...
  console.log(
    `Minimum send delay: ${minSendDelay}ms`
  );

  console.log(
    `Default hourly email limit: ${defaultHourlyLimit}`
  );
});
