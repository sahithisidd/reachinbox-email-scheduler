...
function createBullBoard({ queues, serverAdapter, options = { uiConfig: {} }, }) {
    ...
    // creation time.
    const hasHistoryUsage = Boolean(historyProvider === null || historyProvider === void 0 ? void 0 : historyProvider.getUsage);
    const hasLatencyHistory = Boolean(historyProvider === null || historyProvider === void 0 ? void 0 : historyProvider.getLatency);
    const readOnlyBoard = queues.length > 0 && queues.every((queue) => queue.readOnlyMode);
    const canPurgeHistory = Boolean(historyProvider === null || historyProvider === void 0 ? void 0 : historyProvider.purge) && !readOnlyBoard;
    const apiRoutes = [...routes_1.appRoutes.api];
    if (historyProvider) {
        apiRoutes.push({
            method: 'get',
            route: '/api/metrics/history',
            handler: (0, metricsHistory_1.createMetricsHistoryHandler)(historyProvider),
        });
        if (hasHistoryUsage) {
            apiRoutes.push({
                method: 'get',
                route: '/api/metrics/history/usage',
                handler: (0, metricsHistoryStorage_1.createMetricsHistoryUsageHandler)(historyProvider),
            });
        }
        if (canPurgeHistory) {
            apiRoutes.push({
                ...
                handler: (0, metricsHistoryStorage_1.createMetricsHistoryPurgeHandler)(historyProvider),
            });
        }
        if (hasLatencyHistory) {
            apiRoutes.push({
                ...
                handler: (0, metricsLatency_1.createMetricsLatencyHandler)(historyProvider),
            });
        }
    }
    const finalApiRoutes = options.handlerHooks
        ? apiRoutes.map((route) => ({
            ...route,
            handler: (0, hooks_1.wrapHandlerWithHooks)(route, options.handlerHooks),
        }))
        : apiRoutes;
    serverAdapter
